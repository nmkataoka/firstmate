...
description: >-
  ...
  Use at intake before dispatching a ship task that should carry the dual review (a review-only no-mistakes pipeline pass plus a codex second reviewer), when choosing the review tier for a PR, or before scaffolding a brief with fm-brief.sh --review.
...
After implementation, the crewmate runs a review-only no-mistakes pipeline pass over its branch, then launches one INDEPENDENT fresh-context codex second reviewer over its PR, and finishes with one cleanup pass - explicitly in that order, before the work is ready for the captain.
...
- The implementer IS allowed to triage the codex second reviewer's feedback itself (captain confirmed; it almost never needs help).
- The expensive skill-based workflow runs AT MOST ONCE per PR (~1-2M tokens).
  Any follow-up review after fixes reuses the pipeline review pass, never a repeat of the expensive codex workflow.
...
- Small or frontend-only PRs: `simple` - the codex second reviewer gets a plain review prompt with no skill and no subagent references.
- Everything else: `full` - the codex skill+subagents workflow, at most once.

The tier shapes only the codex second-reviewer pass; the pipeline review pass and the cleanup pass are tier-independent.
