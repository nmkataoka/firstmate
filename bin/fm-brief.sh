...
\`$RESOLVE_VERB\` separately closes an escalated decision or blocker, and only a \`$RESOLVE_VERB\` line carrying that decision's exact key closes it: a later \`done\` or \`working\` event never does, even when the answer is what started that work.
The main firstmate's answer normally writes that closing line at answer time; when a blocker or wait clears WITHOUT an answer from the main firstmate, append \`$RESOLVE_VERB: {how it cleared}\` yourself (keyed with \`[key=<slug>]\` if you opened it with one) as your domain resumes.
...
6. If a decision belongs to a human (product choices, destructive actions),
   ...
   A decision or blocker you opened stays open until a \`$RESOLVE_VERB\` line carrying its exact key lands; a later \`done:\` or \`working:\` line never closes it, even when the answer is what started that work.
   Firstmate's reply normally writes that closing line at answer time; when a blocker or wait clears WITHOUT a firstmate reply, append \`$RESOLVE_VERB: {how it cleared}\` yourself (same \`[key=<slug>]\` if you opened it with one) as you resume.
