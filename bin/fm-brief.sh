...
# Usage: fm-brief.sh <task-id> <repo-name> [--scout] [--herdr-lab] [--review=<full|simple>]
...
6. If a decision belongs to a human (product choices, destructive actions),
   ...
   When firstmate replies or a blocker clears and you resume, append \`$RESOLVE_VERB: {how it was decided or unblocked}\` (add the same \`[key=<slug>]\` if you opened it with one) so the decision or blocker is durably closed and does not keep resurfacing.
