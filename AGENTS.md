...
Shared, tracked material means `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, `crew/`, and public `skills/`.
...
config/branch-prefix  crew task branch prefix override for scaffolded ship briefs; LOCAL, gitignored; absent or empty = default "fm/"; format, normalization, and invalid-value fallback are documented in bin/fm-brief.sh's header; inherited by secondmate homes
...
config/review.env    optional post-implementation reviewer launch overrides; LOCAL, gitignored; read by bin/fm-review-launch.sh with pilot-verified defaults when absent (pr-review-dispatch skill, section 13); per-home, not inherited into secondmate homes
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  <id>/screenshots/  visual-PR screenshots saved locally by the crewmate per crew/review/pr-description-writing.md; survives teardown
...
state/               volatile runtime signals; gitignored
  ...
  <id>.stale-ack     acknowledged-terminal marker written by bin/fm-stale-ack.sh (section 8); suppresses the task's stale wakes while it matches the status log's last line; removed by teardown
...
The same sweep also propagates the primary's declared inheritable config (`config/crew-dispatch.json`, `config/crew-harness`, `config/backlog-backend`, and `config/branch-prefix`; sections 4 and 10) into each live secondmate home's `config/`, so every secondmate's own crewmates, dispatch profiles, backlog backend, and branch prefix stay on the primary's settings.
...
`config/crew-dispatch.json`, `config/crew-harness`, `config/backlog-backend`, and `config/branch-prefix` are inherited into every secondmate home; `config/secondmate-harness` is not, because secondmates never spawn secondmates.
...
**Initialize (`no-mistakes` mode, plus `direct-PR` projects on the review-only pipeline flow):**
...
3. `stale:` the crewmate stopped without reporting; peek the pane (`bin/fm-peek.sh <window>`) to diagnose.
   ...
   After relaying a terminal report for a task that must stay parked awaiting an external event (e.g. a PR merge gating teardown), acknowledge it with `bin/fm-stale-ack.sh <id>` so the watcher absorbs that window's stale wakes instead of re-escalating the parked pane.
   A new status append from that crewmate invalidates the ack and normal handling resumes; signal and check wakes are never suppressed by an ack.
...
Ship briefs also sanction `data/<id>/screenshots/` as the only out-of-worktree write beyond the status file and, for PRs with visual changes, point the crewmate at `crew/review/pr-description-writing.md` for the screenshot evidence rules.
