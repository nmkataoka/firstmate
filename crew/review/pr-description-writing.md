...
Inspect every screenshot before its permanent upload, crop or redact any sensitive content, and skip the upload while retaining only the local save when safe evidence cannot be produced.
Additionally, upload each safe screenshot to the PR base repository using a per-PR draft release as the asset host:
...
This upload applies only when the author has push access to the PR base repository, and the asset links are visible to that repository's collaborators with write-or-higher access viewing them in authenticated browser sessions.
...
Verify an upload from the upload response JSON, never by fetching the asset URL afterward: draft asset URLs are served only to repository collaborators with write-or-higher access in authenticated browser sessions, so anonymous and API-token requests get 404 and a curl 404 does not mean the upload failed.
In the PR description, embed each image with standard Markdown image syntax and put the plain labeled asset link directly beneath it, so the link still works if the inline embed renders broken for viewers; if first real use shows inline embeds broken, trim this guidance to links-only.
