...
## Stage 1 - pipeline review (review-only no-mistakes run)

Never review your own diff in this session; this stage runs a fresh reviewer inside the no-mistakes pipeline.
...
## Stage 2 - codex second reviewer (after stage 1 resolves)

Launch one INDEPENDENT fresh-context codex reviewer whose only inputs are the PR/diff and the tier's prompt.
Run `FM/bin/fm-review-launch.sh <tier> <pr-number> --print` from the worktree root to get the verified launch commands, then run ONLY the printed codex command, capturing its stdout verbatim to a file in your worktree tmp (do not summarize it before saving).
Do not run the printed claude command as a review round - the pipeline review in stage 1 replaced it.
...
Tier `full` (expensive; runs AT MOST ONCE per PR - never repeat, regardless of outcome): the codex prompt points the reviewer at the review skill at `FM/crew/review/diff-review.md` and contains the words "Use subagents", which MUST appear in the prompt.
Tier `simple`: a plain review prompt, no skill, no subagents.

Then triage the codex findings yourself.
...
If your stage 2 fixes were substantive (behavior changes, not nits), run stage 1 once more over the updated branch before moving on.
HARD CAP: two pipeline review runs and one codex pass total.
