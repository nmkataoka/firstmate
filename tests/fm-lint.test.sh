...
test_ci_pins_tasks_axi_version() {
  [ "$(grep -Fxc '          npm install -g tasks-axi@0.2.2' "$CI")" -eq 3 ] \
    || fail "all three portable behavior jobs must install tasks-axi@0.2.2"
  assert_no_grep 'npm install -g tasks-axi$' "$CI" \
    "CI must not retain a floating tasks-axi install"
  pass "CI pins tasks-axi 0.2.2 in all three portable behavior jobs"
}
