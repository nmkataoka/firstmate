...
It only inspects commands where a watcher-arming or checkpoint reference sits in **command position**: `fm-watch-arm.sh`, `fm-watch.sh` (bare), `fm-watch-checkpoint.sh`, or `pkill` (with an `fm-watch` target anywhere in its arguments, quoted or not), at the start of the text or right after a statement separator (`;`, `&`, `|`, a subshell/brace open, a backtick), optionally preceded by env assignments and run-prefix words such as `exec`, `eval`, or `nohup`.
Position is judged on the quote-stripped command text, and additionally inside each quoted payload when the command invokes a nested shell evaluator (`bash -c`, `eval`), so `bash -lc 'bin/fm-watch-arm.sh &'` is still relevant.
Quoted string content and plain arguments never trigger relevance: before this rule (2026-07-10) the anywhere-in-text match false-denied read-only grep/git commands whose search patterns named `fm-watch*` and a `no-mistakes axi respond --instructions "..."` whose prose named the scripts.
