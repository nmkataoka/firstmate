...
It only inspects commands where a watcher-arming or checkpoint reference sits in **command position**: `fm-watch-arm.sh`, `fm-watch.sh` (bare), `fm-watch-checkpoint.sh`, or `pkill` (with an `fm-watch` target anywhere in its arguments, quoted or not), at the start of the text or right after a statement separator (`;`, `&`, `|`, a subshell/brace open, a backtick), optionally preceded by env assignments and run-prefix words such as `exec`, `nohup`, or `timeout` together with their flag/numeric arguments.
Position is judged on the quote-stripped command text, on a separator-neutralized projection that keeps quoted content (so a quoted command word like `"bin/fm-watch-arm.sh" &` cannot dodge the check, while statement separators inside quotes are neutralized and quoted prose cannot fabricate a command-position anchor), and additionally inside each quoted payload when a nested shell evaluator (`bash -c`, `eval`) itself sits in command position, so `bash -lc 'bin/fm-watch-arm.sh &'` is still relevant.
Quoted string content and plain arguments never trigger relevance: before this rule (2026-07-10) the anywhere-in-text match false-denied read-only grep/git commands whose search patterns named `fm-watch*` and a `no-mistakes axi respond --instructions "..."` whose prose named the scripts, and a bare `eval` argument word (a grep pattern, a git pathspec) similarly must not turn quoted segments into command payloads.
Everything else is a fast, silent allow: a raw-substring pre-filter (`fm-watch` or `pkill` anywhere in the quote-delimiter-free command text) short-circuits before any projection work, because every deny needs one of those anchors in some projection and the projections only drop quote delimiters, backslashes, and quoted content - so the per-character projection walkers never run on the ordinary large commands this hook sees on every shell call.

## Primary-checkout scoping

Denies fire only from firstmate's **primary** checkout.
The tracked hook files ship at the repo root, so they are checked out into every worktree of this repo - crewmate/scout task worktrees for firstmate-on-itself work and secondmate homes - but the seatbelt protects the primary's supervision loop, not workers.
The script uses the same primary predicate as `bin/fm-turnend-guard.sh`, keyed off its own location (honoring `FM_ROOT_OVERRIDE`, which the tests use): no `.fm-secondmate-home` marker, `git-dir` equal to `git-common-dir` (a linked worktree's differs), and a firstmate-shaped root (`AGENTS.md`, `bin/`).
Outside the primary - or when git cannot prove it - every command is a silent allow; the check runs lazily, only when a deny is about to fire, so the common allow path never pays the git calls.
...
The arm/checkpoint invocation is blessed in both its repo-relative forms (`bin/`, `./bin/`) and its absolute form (`/.../bin/fm-watch-arm.sh`).
The x-mode source clauses accept the repo-relative `config/x-mode.env` path or the effective absolute path, optionally single-quoted, exactly as `bin/fm-supervision-instructions.sh` renders it into the recipe (it shell-quotes `$CONFIG/x-mode.env`, pinned by `tests/fm-supervision-instructions.test.sh`) - this closes the previously documented gap where the rendered absolute x-mode arm command was denied as bundling.
Leading statements are separated from the final statement by `;` or a newline, matching the separator shape of the documented arm recipe (`AGENTS.md` section 8, `docs/supervision-protocols/grok.md`) - a leading statement joined to the final one with `&&` (e.g. `cd X && exec bin/fm-watch-arm.sh`) is bundling, not the blessed shape, and is denied like any other multi-statement command.
...
It also pins the positional-relevance regressions (watcher-script names in grep/git search patterns, quoted prose arguments, and nested-shell payloads that only grep for the names all allow, while command-position uses including quoted pkill targets still deny), the separator-neutralized projection (quoted and `timeout`-wrapped arm command words still deny, quoted prose with an in-quote semicolon before a script name still allows), the nested-evaluator command-position scoping (a bare `eval` argument word with a quoted watcher path allows, evaluator payloads still deny), the absolute-path blessing (standalone absolute arm/checkpoint and the rendered quoted absolute x-mode clause allow, absolute bundling/backgrounding/redirection still deny), and the primary-checkout scoping (a linked worktree, a marker-bearing secondmate home, and a non-repo root are silent allows in both CLI and stdin modes, while the primary-shaped fixture keeps the full deny behavior).
The allow/deny table runs against a primary-shaped `FM_ROOT_OVERRIDE` fixture so it exercises the full predicate no matter where the suite itself runs.
