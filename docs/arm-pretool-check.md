...
The x-mode source clauses are matched as that literal repo-relative `config/x-mode.env` path only.
...
Known gap: the rendered supervision block substitutes the effective **absolute** x-mode path into that recipe (`bin/fm-supervision-instructions.sh` shell-quotes `$CONFIG/x-mode.env`, pinned by `tests/fm-supervision-instructions.test.sh`), and the literal-path regexes above do not bless it, so the rendered x-mode arm command is denied verbatim (verified empirically 2026-07-10: the relative guarded form allows, the rendered absolute form denies as bundling).
Until the seatbelt learns the effective path, an x-mode arm must use the repo-relative clause from the home root (`cd <home>` first is itself a blessed leading statement).
