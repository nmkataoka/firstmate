...
| `fm-brief.sh`            | Scaffold ship, scout, secondmate-charter, and Herdr-lab briefs, plus fork review tiers and visual-PR evidence guidance |
| `fm-herdr-lab.sh`        | Provision and guardedly operate an isolated, never-default Herdr lab session         |
| `fm-install-herdr.sh`    | Install CI's exact-version Herdr pin with official asset URL, SHA-256, and protocol checks |
| `fm-install-treehouse.sh`| Install CI's exact-version Treehouse pin for real-Herdr E2E that needs spawn worktrees |
| `fm-herdr-ci-cleanup.sh` | Snapshot and tear down only job-owned `fm-lab-*` sessions in the Herdr CI lane       |
| `fm-herdr-session-cleanup.sh` | Reconcile and clean only exact validated Herdr task resources during spawn and teardown |
...
| `fm-doc-audience-check.sh` | Validate maintained-prose inventory, owner pointers, setup routing, and local links |
...
| `fm-review-launch.sh`    | Print or launch this fork's configured post-implementation reviewers                 |
...
| `fm-push-transition-lib.sh` | Own native blocked-transition wakes, exact targets, context, and fire-time beacon stamping |
