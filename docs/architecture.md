...
A `direct-PR` ship task can optionally carry a post-implementation dual review: `bin/fm-brief.sh --review=<full|simple>` pins a firstmate-chosen tier in the brief, and the crewmate runs a review-only no-mistakes pipeline pass (whose gate findings park for firstmate triage), then a claude second-reviewer pass launched with the verified commands `bin/fm-review-launch.sh` owns, and a cleanup pass, per the tracked procedure in `crew/review/` before reporting done.
