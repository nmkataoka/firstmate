...
This fork may select `bin/fm-brief.sh --review=<full|simple>` for a direct-PR ship, which runs the dual-review procedure under `crew/review/` before completion.
Reviewer launch overrides live in local `config/review.env`; [`configuration.md`](configuration.md#post-implementation-review-crewreview-configreviewenv) owns that surface.
