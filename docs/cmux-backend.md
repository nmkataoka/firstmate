...
- Git, a verified agent harness, and the universal tools detected by [`bin/fm-bootstrap.sh`](../bin/fm-bootstrap.sh), following the authoritative [Quick Start requirements](../README.md#requirements); treehouse still provides the worktree, while cmux only provides the session.
...
- **Backend-specific bootstrap detection is absent** - `bin/fm-bootstrap.sh` does not conditionally add `cmux` when a backend selection resolves to cmux, mirroring the same accepted gap already documented for herdr/zellij; `jq` is already covered by universal detection, while the version/tool/reachability gate happens at spawn time and refuses loudly.
