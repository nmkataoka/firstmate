...
- The universal bootstrap toolchain described in [Configuration: Toolchain](configuration.md#toolchain); treehouse still provides the worktree, while zellij only provides the session.
...
- **Backend-specific bootstrap detection is absent.** `bin/fm-bootstrap.sh` does not conditionally add `zellij` when a backend selection resolves to zellij, mirroring the same accepted gap already documented for herdr; `jq` is already covered by universal detection, while the zellij version/tool gate happens at spawn time and refuses loudly.
