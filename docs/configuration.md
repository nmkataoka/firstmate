...
The primary propagates `config/crew-dispatch.json`, `config/crew-harness`, `config/backlog-backend`, and `config/branch-prefix` into secondmate homes at secondmate spawn, during the locked session-start bootstrap secondmate sweep, and during explicit `bin/fm-config-push.sh` runs, so a secondmate's own crewmates, dispatch profiles, backlog backend, and crew branch prefix use the primary values.
...
`crew/review/` holds the tracked crew-facing procedure and prompt files for the post-implementation dual-review workflow (an operator-configured no-mistakes pipeline reviewer plus an independent second reviewer with a different harness/model pairing); generated ship briefs point crewmates at them by absolute path via `bin/fm-brief.sh --review=<full|simple>`, which is verified for direct-PR projects only.
...
`bin/fm-review-launch.sh` owns the verified claude and codex launch commands and per-tier prompts, including the printed claude command the procedure uses for reviewer 2, and reads the optional local, gitignored `config/review.env` for command models, efforts, launch flags, and per-repo guideline links (key reference in the script header).
The current procedure consumes the claude settings for reviewer 2; the codex settings configure only the launcher's retained codex command and do not select or configure pipeline reviewer 1.
...
It uses the same live secondmate discovery and propagation helper as bootstrap, prints each live home's `crew-dispatch.json`, `crew-harness`, `backlog-backend`, and `branch-prefix` result as `pushed`, `unchanged`, `skipped`, or `error`, and exits non-zero only for real propagation errors.
