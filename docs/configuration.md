...
`bin/fm-review-launch.sh` owns the verified reviewer launch commands and per-tier prompts, including the printed claude command the procedure uses for reviewer 2, and reads the optional local, gitignored `config/review.env` for reviewer 2's model, effort, launch flags, and per-repo guideline links (key reference in the script header).
