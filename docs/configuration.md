...
`bin/fm-brief.sh --review=<full|simple>` pins the intake-selected tier and routes the worker to the tracked procedure under `crew/review/`; `crew/review/pr-description-writing.md` separately owns visual-PR screenshot evidence for every ship mode.
...
An absent file uses the pilot-verified defaults documented in [`docs/examples/review.env`](examples/review.env), and the file is not inherited into secondmate homes.
...
The universal toolchain is node, git, gh with GitHub auth via `gh auth login`, curl, jq, no-mistakes v1.31.2 or newer, gh-axi, chrome-devtools-axi, lavish-axi, compatible tasks-axi per "Backlog backend" above, and quota-axi.
This section is the single owner of that universal toolchain list; backend guides' prerequisites point here and add only their backend-specific tools.
In that list, curl and jq support API-backed automation including durable screenshot evidence uploads, no-mistakes runs the validation pipeline, gh-axi, chrome-devtools-axi, and lavish-axi cover GitHub, browser, and rich-review operations, and tasks-axi plus quota-axi back backlog mutations and quota-aware array dispatch.
...
Dispatch profile validation and X mode consume the universal jq and curl dependencies rather than adding conditional bootstrap requirements.
