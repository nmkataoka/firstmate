...
- Git, a verified agent harness, and the universal tools detected by [`bin/fm-bootstrap.sh`](../bin/fm-bootstrap.sh), following the authoritative [Quick Start requirements](../README.md#requirements); treehouse still provides the worktree, while herdr only provides the session.
...
The registration itself is the limit of that confidence: herdr's native agent detection has only been verified to register `claude` and `codex` agents (the harnesses exercised live in this document), and a live agent herdr never registered reads `agent_not_found` -> `no-agent` -> `dead`.
The bootstrap sweep therefore trusts a herdr `dead` verdict only for `claude` and `codex` and downgrades it to `unknown` for any other harness, so an unregistered-but-live secondmate can never be killed and duplicated off a `no-agent` reading.
