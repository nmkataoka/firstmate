...
`bin/fm-push-transition-lib.sh` keeps the durable queue payload annotated while handing supervision the exact unannotated pane target and separate actionable context.
